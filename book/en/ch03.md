...
sources:
  - "SNU Seminar: 260319_Seminar_Junsoo_Jihwan_TactileHand_Manipulation"
  - "Paper summary posts: terry.artlab.ai"
  - "Literature survey results (131 papers)"
  - "Albini et al. (2025) tactile data representation survey"
---
...
The simplest form: sensor readings arranged as a 1D vector. For multi-axis sensors, this becomes a force vector such as [fx, fy, fz]. The OSMO glove [2025] represents tactile state as a 36-dimensional vector from its 12 three-axis sensors.
...
Sensor data projected onto a reference surface. UniTacHand [Zhang et al., 2025] projects tactile data onto a MANO UV map, mapping human and robot hand tactile data into a **shared representation space** (→ Chapter 10.4).
...
The choice of data representation directly impacts learning performance. Wu et al. [2025] proposed Canonical 3D Tactile representations, transforming raw sensor output into a 3D canonical coordinate system to enable task-agnostic transfer (*ICRA 2025*).
...
DexForce [2025] proposed a pipeline using a spring model for kinesthetic teaching that **naturally records force-torque information**. This captures more natural force profiles than teleoperation (→ Chapter 7.4).

### 3.4.3 Autonomous Exploration

The robot autonomously explores the environment to collect tactile data. Throughput is high, but demonstration quality may be lower. PP-Tac [2025] uses physics-based trajectory synthesis for automatic tactile data generation.
...
The scale of tactile datasets remains orders of magnitude smaller than vision datasets. Major public datasets include:
...
9. Wu, C., et al. (2025). Canonical 3D tactile representation for visuo-tactile imitation learning. *ICRA 2025*.
...
13. Zhang, C., Xue, Z., Yin, S., Zhao, B., et al. (2025). UniTacHand: Unified spatio-tactile representation for human to robotic hand skill transfer. *arXiv preprint*. arXiv:2512.21233.

14. Chen, C., Yu, Z., Choi, H., Cutkosky, M., & Bohg, J. (2025). DexForce: Extracting force-informed actions from kinesthetic demonstrations for dexterous manipulation. *IEEE Robotics and Automation Letters*. arXiv:2501.10356.

15. Lin, P., Huang, Y., Li, W., Ma, J., Xiao, C., & Jiao, Z. (2025). PP-Tac: Paper picking using omnidirectional tactile feedback in dexterous robotic hands. *RSS 2025*.
...
21. Yin, J., Qi, H., Wi, Y., Kundu, S., Lambeta, M., Yang, W., Wang, C., Wu, T., Malik, J., & Hellebrekers, T. (2025). OSMO: Open-source tactile glove for human-to-robot skill transfer. *arXiv preprint*. arXiv:2512.08920.
